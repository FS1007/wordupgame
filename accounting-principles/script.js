...
function handleDragEnd(e) {
    ...
    // Check if any words are now complete (reduced delay for immediate feedback)
...
function handleTouchEnd(e) {
    ...
    // Check word completion (reduced delay for immediate feedback)
    setTimeout(() => {
        checkWordCompletion();
    }, 100);
...
function arePiecesConnected(wordPieces, wordData) {
    ...
    for (let i = 0; i < sortedPieces.length - 1; i++) {
        ...
        if (orientation === 'horizontal') {
            ...
            // Increased tolerance to 15px for smoother snapping detection
            if (gap > 15 || verticalAlign > 15) {
